...
class SmartWarehouse:
    ...
    def get_statistics(self):
        ...
        for entry in history.get("sales", []):
            ...
            sales_by_day[day] = sales_by_day.get(day, 0) + entry["data"]["amount"]
        
        for entry in history.get("purchases", []):
            day = entry["timestamp"][:10]
            purchases_by_day[day] = purchases_by_day.get(day, 0) + entry["data"]["amount"]
        
        for entry in history.get("supplies", []):
            day = entry["timestamp"][:10]
            supplies_by_day[day] = supplies_by_day.get(day, 0) + entry["data"]["qty"]
...
<div class="container">
    ...
    <div id="warehouse" class="tab-content active">
        ...
        <div class="warehouse-layout">
            ...
            <div class="row-container">
                <div class="row-info">
                    <div id="cell_l" style="font-size:9px; color:var(--text-muted); margin-bottom:5px;">1 ячейка:<br>{{ stat.max_w }}<div id="kg"> кг </div>| {{ stat.max_v }}<div id="m3"> м³</div></div>
                    <h2 style="margin:0; color:var(--accent); font-size: 22px;">{{ r }}</h2>
                    <div id="gral";style="font-size:9px; color:var(--text-muted); margin-top:5px; margin-bottom:5px;">Вся группа:<br>{{ (stat.max_w * num_cells_in_row)|round(1) }} кг | {{ (stat.max_v * num_cells_in_row)|round(2) }} м³</div>
    ...
    <div id="orders" class="tab-content">
        ...
        <form id="order-form" action="/add_order" method="POST" class="form-row">
            ...
            <div style="grid-column: span 2;">
                <label data-key="f_order_items_label">Товары заказа:</label>
                <div style="display: grid; grid-template-columns: 1fr 100px; gap: 10px; margin-top: 10px;">
                    <input type="text" id="order-item-addr" placeholder="Адрес (A1) или название товара" style="padding:10px; border:1px solid var(--border); border-radius:8px; background:var(--bg-card); color:var(--text-main);">
            ...
            <select name="status" style="padding:10px; border:1px solid var(--border); border-radius:8px; background:var(--bg-card); color:var(--text-main); font-size: 12px; grid-column: span 2;" id="order-status">
                ...
                <option value="returned" data-key="order_status_returned">Возврат</option>
            </select>
                ...
                <tr>
                    ...
                    <td>
                        <select class="status-badge status-{{ order.status }}" onchange="updateOrderStatus('{{ order_id }}', this.value)" style="border:none; background:transparent; padding:0; cursor:pointer;">
                            ...
                            <option value="returned" {% if order.status == 'returned' %}selected{% endif %} data-key="order_status_returned">Возврат</option>
                        </select>
                    </td>
    ...
    <div id="statistics" class="tab-content">
        ...
        <div class="stat-grid">
            ...
            <div class="stat-card">
                <h3 data-key="stat_inventory">📈 Остатки</h3>
...
<div class="modal-content" onclick="event.stopPropagation()">
    ...
    <form id="cell-edit-form">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 15px;">
            ...
            <div>
                <label style="font-weight: bold; display: block; margin-bottom: 5px;" data-key="f_price_label">Цена:</label>
        ...
        <div style="margin-bottom: 15px;">
            <label style="font-weight: bold; display: block; margin-bottom: 5px;">Вывезти (шт):</label>
...
const i18n = {
    ru: {
        ...
        m_apply: "Сохранить", m_clear: "Освободить", m_minus_ph: "Отгрузить шт.", nav_exit: "Выход", btn_logout: "Выход", btn_add: "Добавить", btn_create: "Создать заказ",
    ...
    en: {
        ...
        m_apply: "Save", m_clear: "Clear", m_minus_ph: "Dispatch qty", nav_exit: "Logout", btn_logout: "Logout", btn_add: "Add", btn_create: "Create Order",
    ...
    cn: {
        ...
        m_apply: "保存", m_clear: "清空", m_minus_ph: "出库数量", nav_exit: "退出", btn_logout: "退出", btn_add: "添加", btn_create: "创建订单",
...
function updateCurrencyDisplay() {
    ...
    document.querySelectorAll('[id^="balance-"]').forEach(el => {
        const balance = parseFloat(el.textContent) * currencyRates[currentCurrency].rate;
        el.textContent = balance.toFixed(2) + ' ' + symbol;
    });
}
...
function doSearch() {
    ...
    for(let addr in cellsData) {
        ...
        if(item && (item.name.toUpperCase().includes(q) || addr.includes(q))) {
            ...
            html += `<div class="search-item" onclick="openCellEditor('${addr}', '${item.name}', ${item.qty}, ${item.price}, ${item.weight}, ${item.vol}, '${item.expiry || ''}')">
                <b>${addr}</b> - ${item.name} (${item.qty} ${i18n[currentLang].unit_pcs})<br>
                <small>${getCurrencySymbol()}${price} | ${item.weight}kg | ${item.vol}m³ | ${item.expiry || 'без сроков'}</small>
            </div>`;
        }
    }
...
function addNewGroup() {
    ...
    if(available.length === 0) {
        alert(i18n[currentLang].error_invalid);
        return;
    }
...
function saveCellChanges() {
    ...
    if (exportQty > 0) {
        window.location.href = `/update_item/${addr}?qty=${qty - exportQty}&name=${name}&price=${price}&weight=${weight}&vol=${vol}&expiry=${expiry}`;
    } else {
        window.location.href = `/update_item/${addr}?qty=${qty}&name=${name}&price=${price}&weight=${weight}&vol=${vol}&expiry=${expiry}`;
    }
}

function fullDeleteCell() {
    const addr = document.getElementById('m-addr-input').value;
    if(confirm("Освободить " + addr + "?")) {
        window.location.href="/delete/"+addr;
    }
}
...
function addOrderItem() {
    const addr = document.getElementById('order-item-addr').value.trim();
    ...
    const itemKey = addr.toUpperCase();
    let found = false;
    
    // Проверка по адресу ячейки
    if (cellsData[itemKey]) {
        found = true;
    } else {
        // Проверка по названию товара
        for (let cell in cellsData) {
            if (cellsData[cell] && cellsData[cell].name.toUpperCase() === itemKey) {
                found = true;
                orderItemsBuffer[cell] = qty;
                break;
            }
        }
        if (!found) {
            showError(i18n[currentLang].error_item_not_found + addr);
            return;
        }
    }
    
    if (found && itemKey in cellsData) {
        orderItemsBuffer[itemKey] = qty;
    }
    ...
    for (let addr in orderItemsBuffer) {
        ...
        if (item) {
            ...
            itemDiv.innerHTML = `
                <span class="addr">${addr}</span>:
                <span class="qty">${qty} шт</span> ×
                <span class="price">${convertPrice(item.price)} ${getCurrencySymbol()} = ${convertPrice(itemPrice)} ${getCurrencySymbol()}</span>
                <button type="button" onclick="removeOrderItem('${addr}')" style="float:right; background:var(--danger); color:white; border:none; padding:3px 8px; border-radius:4px; cursor:pointer;">✕</button>
            `;
    ...
    chartsInstance[canvasId] = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: type === 'doughnut' ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)',
                borderWidth: 1,
                tension: 0.4
            }]
        },
...
function renderStatsDetails(data) {
    ...
    html += `
        <div style="background: rgba(46, 213, 115, 0.1); padding: 15px; border-radius: 8px; border-left: 4px solid #2ed573;">
            <div style="font-size: 12px; font-weight: bold;">${i18n[currentLang].stat_supplies}</div>
            <div style="font-size: 24px; font-weight: bold; color: #2ed573;">${totalSupplies}</div>
        </div>
        <div style="background: rgba(0, 191, 255, 0.1); padding: 15px; border-radius: 8px; border-left: 4px solid #00bfff;">
            <div style="font-size: 12px; font-weight: bold;">${i18n[currentLang].stat_sales}</div>
            <div style="font-size: 24px; font-weight: bold; color: #00bfff;">${convertPrice(totalSales)} ${getCurrencySymbol()}</div>
        </div>
        <div style="background: rgba(149, 117, 205, 0.1); padding: 15px; border-radius: 8px; border-left: 4px solid #9575cd;">
            <div style="font-size: 12px; font-weight: bold;">${i18n[currentLang].stat_inventory}</div>
            <div style="font-size: 24px; font-weight: bold; color: #9575cd;">${totalInventory}</div>
        </div>
    `;
...
function renderHistoryList(historyData) {
    ...
    for (const entry of historyData) {
        ...
        let content = '';
        if (typeClass === 'supply') {
            content = `${entry.data.name} (${entry.data.qty} шт) → ${entry.data.addr}`;
        } else if (typeClass === 'sale') {
            content = `Продажа: ${entry.data.client} - ${convertPrice(entry.data.amount)} ${getCurrencySymbol()}`;
        } else if (typeClass === 'purchase') {
            content = `Покупка: ${convertPrice(entry.data.amount)} ${getCurrencySymbol()}`;
        } else if (typeClass === 'transaction') {
            content = `Транзакция: ${entry.data.description}`;
        }
...
"""
# ===== ЧАСТЬ 11: FLASK МАРШРУТЫ (часть 1/4) =====
...
# ===== ЧАСТЬ 12: FLASK МАРШРУТЫ (часть 2/4) =====
    ...
    if wh:
        try:
            ...
            if request.method == 'POST':
                ...
                minus = request.form.get('minus', '0')
            else:
                ...
                minus = request.args.get('minus', '0')
            ...
            if wh.cells[addr]:
                ...
                if m > 0:
                    add_history_entry(wh.username, "purchases", {
                        "addr": addr,
                        "qty": m,
                        "amount": wh.cells[addr]['price'] * m if wh.cells[addr] else 0
                    })
...
# ===== ЧАСТЬ 13: FLASK МАРШРУТЫ (часть 3/4) =====
...
# ===== ЧАСТЬ 14: FLASK МАРШРУТЫ (часть 4/4) - НОВЫЕ ФУНКЦИИ =====
