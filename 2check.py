...
<head>
    ...
    <style>
        ...
        .group-row { display: grid; grid-template-columns: 80px 100px 150px 1fr; gap: 10px; align-items: center; background: var(--bg-card); padding: 10px; border-radius: 8px; border: 1px solid var(--border); font-size: 12px; }
        ...
        <form id="settings-form" method="POST" action="/update_groups">
            ...
            <table class="limits-table">
                ...
                <tr>
                    ...
                    <td><input type="number" name="row_w[]" value="{{ warehouse.rows_limits[r].w }}" style="width:70px; padding: 6px;"></td>
                    <td><input type="number" name="row_v[]" value="{{ warehouse.rows_limits[r].v }}" step="0.1" style="width:70px; padding: 6px;"></td>
                </tr>
                {% endfor %}
            </table>
